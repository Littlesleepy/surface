...
export function mockPanleInited(mock: IMockPanleState) {
  ...
  if (!Object.hasOwn(vice.form.value, "playAudio"))
    vice.form.value.playAudio = false;
  if (!Object.hasOwn(vice.form.value, "playSpeed"))
    vice.form.value.playSpeed = 7;
  ...
  device.elements.value.forEach((el) => {
    if (hide.includes(el.paramName)) el.show = false;
  });
}
